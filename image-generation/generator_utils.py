...
######### GENERAL ####################################################################################
######### GENERAL ####################################################################################
def convert_PIL2CV(im):
    im = im.convert('RGB')
    im = np.array(im)
    return im[:, :, ::-1].copy()


def convert_CV2PIL(im):
...
def open_im(img):
    ...
    kernel = np.ones((3, 3), np.uint8)
    im = cv2.dilate(img, kernel, iterations=ran)
    return im


# def change_color_temp(img, ratio):
#     im = img.copy()
#     if ratio > 0:
#         im[:, :, 0][im[:, :, 0] < 254-ratio] = im[:, :, 0][im[:, :, 0] < 254-ratio ] + ratio
#         im[:, :, 1][im[:, :, 1] < 254-ratio] = im[:, :, 1][im[:, :, 1] < 254-ratio ] + ratio
#         im[:, :, 0][im[:, :, 2] < 254-ratio] = im[:, :, 2][im[:, :, 2] < 254-ratio ] + ratio
#     else:
#         ratio = -ratio
#         im[:, :, 0][im[:, :, 0] > ratio] = im[:, :, 0][im[:, :, 0] > ratio ] - ratio
#         im[:, :, 1][im[:, :, 1] > ratio] = im[:, :, 1][im[:, :, 1] > ratio ] - ratio
#         im[:, :, 2][im[:, :, 2] > ratio] = im[:, :, 2][im[:, :, 2] > ratio ] - ratio
#     return im

def overlay_red_mark_cv(img1, dm):
    # Origin img
    img2 = img1.copy()
    h, w, c = img1.shape

    # red wmark
    dm = resize_cv(dm, int(h // round(random.uniform(8.0, 12.0), 2)))
    dm = transform(dm)
    dm = rotate_bound(dm, random.randint(-45, 45))
    cv2.imwrite('t_dm.jpg', dm)

    rows, cols, channels = dm.shape
    cx = int(w // round(random.uniform(1, 1.5), 2))
    cy = int(h // round(random.uniform(2, 3), 2))
    print(cx, cy)

    # Overlay
    roi = img1[cx:cx + rows, cy:cy + cols]
    mask = cv2.cvtColor(dm, cv2.COLOR_BGR2GRAY)
    mask[mask < 10] = 255
    mask[mask > 180] = 255
    mask[mask <= 180] = 0
    ran = random.randint(-50, 5)
    dm = change_color_temp(dm, ran)
    dm[:, :, 0][mask > 250] = 255
    dm[:, :, 1][mask > 250] = 255
    dm[:, :, 2][mask > 250] = 255

    cv2.imwrite('ths.jpg', mask)
    mask_inv = cv2.bitwise_not(mask)
    img1_bg = cv2.bitwise_and(roi, roi, mask=mask)
    img2_fg = cv2.bitwise_and(dm, dm, mask=mask_inv)

    dst = cv2.add(img1_bg, img2_fg)
    img1[cx:cx + rows, cy:cy + cols] = dst

    return cv2.addWeighted(img1, .6, img2, .4, 0)
...
######### TRANSFORM ####################################################################################
######### TRANSFORM ####################################################################################
def resize_cv(im, new_heigh):
    y, x, c = im.shape
    ratio = new_heigh / y
    y_new = round(y * ratio)
    x_new = round(x * ratio)
    return cv2.resize(im, (x_new, y_new))


def resize_PIL(im, new_heigh):
    x, y = im.size
    ratio = new_heigh / y
    y_new = round(y * ratio)
    x_new = round(x * ratio)
    return im.resize((x_new, y_new), Image.ANTIALIAS)
...
def wb_color(im, ratio):
    if ratio > 0:
        im[:, :, 0][im[:, :, 0] < 255 - ratio] = im[:, :, 0][im[:, :, 0] < 255 - ratio] + ratio
        im[:, :, 2][im[:, :, 2] > ratio] = im[:, :, 2][im[:, :, 2] > ratio] - ratio
    else:
        ratio = -ratio
        im[:, :, 0][im[:, :, 0] > ratio] = im[:, :, 0][im[:, :, 0] > ratio] - ratio
        im[:, :, 2][im[:, :, 2] < 255 - ratio] = im[:, :, 2][im[:, :, 2] < 255 - ratio] + ratio
    return im


def wb_color(img, ratio):
    im = img.copy()
    if ratio > 0:
        im[:, :, 0][im[:, :, 0] < 254 - ratio] = im[:, :, 0][im[:, :, 0] < 254 - ratio] + ratio
        im[:, :, 1][im[:, :, 1] < 254 - ratio] = im[:, :, 1][im[:, :, 1] < 254 - ratio] + ratio
...
if __name__ == '__main__':
    ...
    dir_in = 'DOB/b/gif/'
    # dir_out = 'fake_name_id/out1'
    for f in os.listdir(dir_in):
        print(f)
        im = Image.open(dir_in + f)
        im = convert_PIL2CV(im)
        cv2.imwrite(dir_in + f.replace('.gif', '.jpg'), im)

        # img = cv2.imread(dir_in + '/' + f)

    #         ######### Add effects #######

    #         # im = noise_blur(img.copy())
    #         # # im = noise_blur(im)
    #         # im = adjust_gamma(im)
    #         # im = transform(im)
    #         # # im = noise_salt(img)
    ...
    #         # im, _ = find_boundingBox(im)
    #         # im = noise_pepper(im)

    #         im = open_im(img)`
    #         #############################

    #         file_save = dir_out + '/' + f[:-4] + 'o.jpg'
    #         # file_save = dir_out + '/' + f.replace("s", "m")
    #         print(file_save)
...
def remove_shadow(img):
    ...
    for plane in rgb_planes:
        ...
        norm_img = cv2.normalize(diff_img, diff_img.copy(), alpha=0, beta=255, norm_type=cv2.NORM_MINMAX,
                                 dtype=cv2.CV_8UC1)
